...
function StatCard({ label, value, tone = 'default' }: StatCardProps) {
  ...
  return (
    <div className="border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/50">
      <p className={`text-xs font-semibold uppercase ${palette.badge.split(' ').filter(c => c.startsWith('text-')).join(' ')}`}>
        {label}
      </p>
